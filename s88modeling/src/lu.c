...
void strip (FILE * agrfp,
	    float xmin,
	    float xmax,
	    interface_t * s1, interface_t * s2, unsigned int cor);
...
int agr_write (gchar * agrfilename, lu_t * lu, struct s88 *p)
{
	...
	for (i = 0; i < lu->nint - 1; i++)
		strip (agrfp, p->xmin, p->xmax, &(lu->interf[i]),
		       &(lu->interf[i + 1]), i + 2 + p->land);
	...
	if (p->vel != NULL) {
		...
		for (ix = 0; ix < p->nx; ix++) {
			...
			x = p->xmin + ix * (p->xmax - p->xmin) / (p->nx -
								  1);

			for (ilayer = 0; ilayer < lu->nint; ilayer++) {
				depth[ilayer] =
				    seval ((int *) &(lu->interf[ilayer].n),
					   &x, lu->interf[ilayer].x,
					   lu->interf[ilayer].z,
					   lu->interf[ilayer].b,
					   lu->interf[ilayer].c,
					   lu->interf[ilayer].d);
...
strip (FILE * agrfp,
       float xmin,
       float xmax, interface_t * s1, interface_t * s2, unsigned int cor)
{
	int n, i;
	float *x, *z, lambda, xx;
...
void SetAGRMyPalette (void)
{
	...
	AGR_colorname =
	    (char **) malloc (AGR_numberofcolors * sizeof (char *));

	for (i = 0; i < AGR_numberofcolors; i++) {
		AGR_palette[i] =
		    (unsigned short *) malloc (3 *
	...
	AGR_palette[9][1] = 69;
	AGR_palette[9][2] = 19;
	AGR_palette[10][0] = 244;
	AGR_palette[10][1] = 164;
	AGR_palette[10][2] = 96;
	AGR_palette[11][0] = 205;
	AGR_palette[11][1] = 92;
	AGR_palette[11][2] = 92;
	AGR_palette[12][0] = 233;
	AGR_palette[12][1] = 150;
	AGR_palette[12][2] = 12;
	AGR_palette[13][0] = 240;
	AGR_palette[13][1] = 248;
	AGR_palette[13][2] = 255;
	AGR_palette[14][0] = 250;
	AGR_palette[14][1] = 235;
	AGR_palette[14][2] = 215;
	AGR_palette[15][0] = 255;
	AGR_palette[15][1] = 228;
	AGR_palette[15][2] = 181;
	AGR_palette[16][0] = 255;
	AGR_palette[16][1] = 218;
	AGR_palette[16][2] = 185;
	AGR_palette[17][0] = 245;
	AGR_palette[17][1] = 245;
	AGR_palette[17][2] = 220;
	AGR_palette[18][0] = 245;
	AGR_palette[18][1] = 222;
	AGR_palette[18][2] = 179;
	AGR_palette[19][0] = 210;
	AGR_palette[19][1] = 180;
	AGR_palette[19][2] = 140;
	AGR_palette[20][0] = 220;
	AGR_palette[20][1] = 220;
	AGR_palette[20][2] = 220;
	AGR_palette[21][0] = 47;
	AGR_palette[21][1] = 79;
	AGR_palette[21][2] = 79;
	AGR_palette[22][0] = 105;
	AGR_palette[22][1] = 105;
	AGR_palette[22][2] = 105;
	AGR_palette[23][0] = 112;
	AGR_palette[23][1] = 128;
	AGR_palette[23][2] = 144;
	AGR_palette[24][0] = 119;
	AGR_palette[24][1] = 136;
	AGR_palette[24][2] = 153;
	AGR_palette[25][0] = 190;
	AGR_palette[25][1] = 190;
	AGR_palette[25][2] = 190;
	AGR_palette[26][0] = 205;
	AGR_palette[26][1] = 201;
	AGR_palette[26][2] = 201;
	AGR_palette[27][0] = 139;
	AGR_palette[27][1] = 137;
	AGR_palette[27][2] = 137;
	AGR_palette[28][0] = 255;
	AGR_palette[28][1] = 246;
	AGR_palette[28][2] = 14;
	AGR_palette[29][0] = 238;
	AGR_palette[29][1] = 230;
	AGR_palette[29][2] = 13;
	AGR_palette[30][0] = 205;
	AGR_palette[30][1] = 198;
	AGR_palette[30][2] = 11;
	AGR_palette[31][0] = 139;
	AGR_palette[31][1] = 134;
	AGR_palette[31][2] = 78;
	AGR_palette[32][0] = 70;
	AGR_palette[32][1] = 130;
	AGR_palette[32][2] = 180;
	AGR_palette[33][0] = 100;
	AGR_palette[33][1] = 149;
	AGR_palette[33][2] = 237;
	AGR_palette[34][0] = 72;
	AGR_palette[34][1] = 61;
	AGR_palette[34][2] = 139;
	AGR_palette[35][0] = 46;
	AGR_palette[35][1] = 139;
	AGR_palette[35][2] = 87;
	AGR_palette[36][0] = 85;
	AGR_palette[36][1] = 107;
	AGR_palette[36][2] = 47;
	AGR_palette[37][0] = 189;
	AGR_palette[37][1] = 183;
	AGR_palette[37][2] = 107;
	AGR_palette[38][0] = 250;
	AGR_palette[38][1] = 250;
	AGR_palette[38][2] = 210;
	AGR_palette[39][0] = 255;
	AGR_palette[39][1] = 215;
	AGR_palette[39][2] = 0;
	AGR_palette[40][0] = 218;
	AGR_palette[40][1] = 165;
	AGR_palette[40][2] = 32;
	AGR_palette[41][0] = 184;
	AGR_palette[41][1] = 134;
	AGR_palette[41][2] = 11;
	AGR_palette[42][0] = 255;
	AGR_palette[42][1] = 165;
	AGR_palette[42][2] = 0;
	AGR_palette[43][0] = 255;
	AGR_palette[43][1] = 140;
	AGR_palette[43][2] = 0;
	AGR_palette[44][0] = 255;
	AGR_palette[44][1] = 0;
	AGR_palette[44][2] = 0;
	AGR_palette[45][0] = 0;
	AGR_palette[45][1] = 255;
	AGR_palette[45][2] = 0;
	AGR_palette[46][0] = 0;
	AGR_palette[46][1] = 0;
	AGR_palette[46][2] = 255;
	AGR_palette[47][0] = 255;
	AGR_palette[47][1] = 255;
	AGR_palette[47][2] = 0;
	AGR_palette[48][0] = 0;
	AGR_palette[48][1] = 255;
	AGR_palette[48][2] = 255;
	AGR_palette[49][0] = 255;
	AGR_palette[49][1] = 0;
	AGR_palette[49][2] = 255;
	AGR_palette[50][0] = 148;
	AGR_palette[50][1] = 0;
	AGR_palette[50][2] = 211;
	AGR_palette[51][0] = 64;
