/*   suxkill - Zero out seismic traces
...
char *sdoc[] = {
"                                                                             ",
" SUXKILL -  X-windows zero out traces                                        ",
"                                                                             ",
" suxkill <infile >outfile                                                    ",
"                                                                             ",
...
"    Range off/on            default is Range off, click to enable Sukill     ",
"                            with range                                       ",
"    One Section/All Section default is One Section, click to enable Sukill   ",
"                            on all Sections                                  ",
"    Kill traces/Zero traces Kill traces changes tr.id of the trace (tr.id=2) ",
"    Sukill                  Kill selected picks                              ",
...
void draw_command_bar(int winwidth, TextSet *filename_input,
		      ...
		      int cross_mode,int range_mode, int erase_mode, int all_mode,cwp_String header_key);
...
void sukill(pick_t **apick,int *pickdimend,int *range_mode,int *erase_mode,int *all_mode,int count,int nsec,unsigned long traces[BUFSIZ]);  
void suxkill(int min, int count_kill,int mode);
int check_traces(int count, unsigned long traces[BUFSIZ], int min, int count_kill);
...
/******************************************************************************************************/
	...
	win = xNewWindow(dpy,xbox,ybox,
			 wbox+COMMAND_WIDTH,hbox,(int) black,(int) white,"Suxkill");
	...
	while(num_wiggles|(~num_wiggles)/*True*/) {
		...
		/* else if window exposed */
			...
			draw_command_bar(winwidth,filename_input,pick_fname,
					 control_mode,edit_mode, cross_mode,range_mode,erase_mode,all_mode,header_key);
		      ...
		      int cross_mode,int range_mode, int erase_mode,int all_mode,cwp_String header_key)
{
	...
	NewButton(window,winwidth-COMMAND_WIDTH-20,(BUTTON_HEIGHT-5)*16,
			BUTTON_WIDTH+12,BUTTON_HEIGHT-5,
			UP,grey_color,black_color,
			(erase_mode) ? "Zero Trace" : "Kill Trace",
			char_width,char_height,RELATIVE);					
	...
	NewButton(window,winwidth-COMMAND_WIDTH-20,(BUTTON_HEIGHT-5)*18,
			BUTTON_WIDTH+12,BUTTON_HEIGHT-5,
			UP,grey_color,black_color,
			"Sukill",char_width,char_height,RELATIVE);
		   ...
		   float *perc)
{
	...
	if(mx>winwidth-COMMAND_WIDTH-20 && mx<winwidth-COMMAND_WIDTH+BUTTON_WIDTH-10) {
		if(my>BUTTON_HEIGHT*1-(char_height+30) && my<BUTTON_HEIGHT*1) {
			SetCurrentTextSet(filename_input,UP);
...
if(my>(BUTTON_HEIGHT-5)*2 && my<(BUTTON_HEIGHT-5)*3) {    /*LOAD*/
		...
		if(my>(BUTTON_HEIGHT-5)*12 && my<(BUTTON_HEIGHT-5)*13) {  
			char str[20];
			*all_mode = !(*all_mode);	
			if(*all_mode){
				 strcpy(str,"All ");
		...
		if(my>(BUTTON_HEIGHT-5)*14 && my<(BUTTON_HEIGHT-5)*15) {  
		...
		if(my>(BUTTON_HEIGHT-5)*16 && my<(BUTTON_HEIGHT-5)*17) {  
		...
		if(my>(BUTTON_HEIGHT-5)*18 && my<(BUTTON_HEIGHT-5)*19) {   /*sukill*/
			NewButton(window,winwidth-COMMAND_WIDTH-20,(BUTTON_HEIGHT-5)*18,
				BUTTON_WIDTH+12,BUTTON_HEIGHT-5,
				UP,grey_color,black_color,
				"Sukill",char_width,char_height,RELATIVE);
				sukill(apick,pickdimend,range_mode,erase_mode,all_mode,*count,nsec,traces);
...
void read_cmd(int sec,unsigned long traces[BUFSIZ]){
	unsigned long mini;
	int j,count_kill,mode,sec_read;
	FILE *comandfp;
	
	if((comandfp=fopen(comandfile,"r+")) != NULL){
		while (!feof(comandfp)) {
			j=fscanf(comandfp,"%d min %lu count %d mode %d\n",&sec_read,&mini,&count_kill,&mode);
			if(sec_read==sec){
				suxkill(mini,count_kill,mode);	
				new_section_file();
  			}	 				
		}
    }
...
void sukill(pick_t **apick,int *pickdimend,int *range_mode,int *erase_mode,int *all_mode,int count,int nsec,unsigned long traces[BUFSIZ])
{	
	unsigned long ponto,mini,maxi;
	int diff,i,j,k;
	FILE *comandfp;
	if(*pickdimend>0){
		comandfp = fopen(comandfile, "a");
		if(*range_mode==1){
			if((*apick)[0].picked==TRUE){
				mini = maxi = (*apick)[0].x2;
				for(k=1;k<*pickdimend;k++){
					if(mini>(*apick)[k].x2){
						mini = (unsigned long)(*apick)[k].x2 ;
					}
					if(maxi<(*apick)[k].x2){
						maxi = (unsigned long)(*apick)[k].x2;
					}
				}
				diff = maxi-mini+1;
				if(check_traces(count,traces,mini,diff)){
					fprintf(comandfp,"%d\n",count);
					fprintf(comandfp,"min %lu count %d mode %d\n",mini,diff,*erase_mode);
				}
				else warn("Invalid Picks for section %d",count);
				if(*all_mode==1){
					for(j=0;j<nsec-1;j++){
						if(count!=j){
							if(check_traces(j,traces,mini,diff)){
								fprintf(comandfp,"%d\n",j);
								fprintf(comandfp,"min %lu count %d mode %d\n",mini,diff,*erase_mode);
							}
							else warn("Invalid Picks for section %d",j);
						}
					}
				}
			}
			...
			for(i=0;i<*pickdimend;i++){
				if((*apick)[i].picked==TRUE){
					ponto = (unsigned long)(*apick)[i].x2;
					if(check_traces(count,traces,ponto,1)){			
						fprintf(comandfp,"%d\n",count);
						fprintf(comandfp,"min %lu count %d mode %d\n",ponto,1,*erase_mode);
					}
					else warn("Invalid Picks for section %d",count);
					if(*all_mode==1){
						for(j=0;j<nsec-1;j++){
							if(count!=j){
								if(check_traces(j,traces,ponto,1)){
									fprintf(comandfp,"%d\n",j);
									fprintf(comandfp,"min %lu count %d mode %d\n",ponto,1,*erase_mode);
								}
								else warn("Invalid Picks for section %d",j);
...
void suxkill(int min, int count_kill,int mode){
	...
	for (itr = 1; itr < min; ++itr) {
		if (fgettr(infp,&tr)) fputtr(outfp,&tr);
...
int check_traces(int count, unsigned long traces[BUFSIZ], int min, int count_kill){
	...
	if(n2<min) return 0;
		...
		if(n2<min+count_kill-1) return 0;
		else return 1;
	}
}
